...
import { CoachTip } from "@/components/coach-tips/CoachTip";
...
function TextDisplay({
  ...
  return (
    <div className="max-w-[343px] w-full min-w-[343px] flex items-center justify-between gap-3 relative">
      <div>
        ...
        {helperText && (
          <>
            <ContextualAlert label={helperText} variant="neutral" />
          </>
        )}
      </div>
      <div>
        {message && (
          <div>
            {showmessage === true && (
              <div className={cn("absolute bottom-[119%] translate-x-[-60.5%]")}>
                <CoachTip onClose={hideMessageFunc} position="bottom" text={[message]} />
              </div>
            )}
            <div
              onClick={showMessageFunc}
              className="w-10 h-10 flex items-center justify-center rounded-full cursor-pointer absolute p-2 top-[2px] right-3"
            >
              <svg
