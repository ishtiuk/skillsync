...
from app.services.user import get_active_user, user_service
...
def login(request: LoginRequest, db: Session = Depends(get_db)):
    try:
        if user_service.validate_user(email=request.email, password=request.password, db=db):
...
def create_user(user_data: UserCreateRequest, db: Session = Depends(get_db)):
    try:
        user_data = user_service.create_user_in_db(db=db, user=user_data)
        access_token = create_access_token(subject=user_data.email)
        return JSONResponse(
            content={
                "message": "User created successfully",
                "user": jsonable_encoder(UserResponse(**jsonable_encoder(user_data))),
...
def get_current_user(current_user: Users = Depends(get_active_user)):
    try:
        return JSONResponse(
...
def update_password(
    ...
    try:
        updated_user = user_service.update_password(
            db=db, email=request.email, new_password=request.password
        )
        return JSONResponse(
            content=jsonable_encoder(UserResponse(**(jsonable_encoder(updated_user)))),
            status_code=status.HTTP_200_OK,
...
def add_experience(
    job_exp: CreateExp,
    db: Session = Depends(get_db),
    user: Users = Depends(get_active_user),
):
    try:
        user_service.add_experience(db=db, user=user, job_exp=job_exp)
        return JSONResponse(
...
def get_experiences(db: Session = Depends(get_db), user: Users = Depends(get_active_user)):
    try:
        job_exps = user_service.get_experiences(db=db, user=user)
        response = [
