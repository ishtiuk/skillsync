...
# from app.schemas.careerforge import CoverLetterRequest, CoverLetterResponse
...
#     file: UploadFile, db: Session = Depends(get_db), current_user: Users = Depends(get_active_user)
# ):
...
# async def match_jobs(db: Session = Depends(get_db), current_user: Users = Depends(get_active_user)):
...
# @cover_letter_router.post("/cover-letter/generate", response_model=CoverLetterResponse)
...
# ):
#     try:
#         generated_cover_letter = cover_letter_service.get_cover_letter(
#             cover_letter_request=cov_data, db=db, user=current_user
#         )
#         return JSONResponse(
