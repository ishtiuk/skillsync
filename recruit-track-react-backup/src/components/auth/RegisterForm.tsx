  ...
  const onSubmit = async (data: RegisterFormData) => {
    ...
    try {
      ...
      toast({
        ...
        description: `Your ${data.platform === 'careerforge' ? 'CareerForge' : 'TalentHub'} account has been created.`,
      });
    } catch (error) {
      toast({
    ...
    <Card className={`border-0 shadow-lg hover-gradient-scale ${
      ...
      <CardContent className="p-4">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-2.5">
          ...
          <Button
            type="submit"
            className={`w-full mt-3 hover-gradient-scale ${
              isCareerForge
        ...
        <div className="mt-3 text-center">
          <p className="text-xs text-gray-600">
            Demo: Use any valid email and password (6+ chars)
          </p>
        </div>
      </CardContent>
    </Card>
